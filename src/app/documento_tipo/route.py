...
async def documento_tipo_list(request: Request):
    try:
        data = await api_backend.get_documento_tipo(filters={},token = request.state.token)
...
async def documento_tipo_form(request: Request):
    try:
        ...
        if(len(request.query_params) !=0 ):
            documento_tipo_data = await api_backend.get_documento_tipo(filters=request.query_params,token = request.state.token)
...
async def documento_tipo_insert(request: Request):
    try:
        ...
        documento_tipo_data = await api_backend.post_documento_tipo(data=data,token = request.state.token)
...
async def documento_tipo_update(request: Request,id:int):
    try:
        data = dict(await request.form())
        await api_backend.patch_documento_tipo(id=id,data=data,token = request.state.token)

...
async def endereco_list(request: Request):
    try:
        data = await api_backend.get_endereco(filters={},token = request.state.token)
...
async def endereco_form(request: Request):
    try:
        ...
        if(len(request.query_params) >1 ):
            endereco_data = await api_backend.get_endereco(filters={"id":request.query_params["id"]},token = request.state.token)
...
async def endereco_insert(request: Request):
    try:
        ...
        endereco_data = await api_backend.post_endereco(data=data,token = request.state.token)
...
async def endereco_update(request: Request,id:int):
    try:
        data = dict(await request.form())
        await api_backend.patch_endereco(id=id,data=data,token = request.state.token)

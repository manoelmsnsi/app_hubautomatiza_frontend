...
async def usuario_list(request: Request):
    try:
        data = await api_backend.get_usuario(filters={},token = request.state.token)
...
async def usuario_form(request: Request):
    try:
        ...
        if(len(request.query_params) !=0 ):
            usuario_data = await api_backend.get_usuario(filters=request.query_params,token = request.state.token)         
...
async def usuario_insert(request: Request):
    try:
        ...
        usuario_data = await api_backend.post_usuario(data=data,token = request.state.token)
...
async def usuario_update(request: Request,id:int):
    try:
        data = dict(await request.form())
        await api_backend.patch_usuario(id=id,data=data,token = request.state.token)

...
async def acesso_list(request: Request):
    try:
        # data = await api_backend.get_acesso(filters={},token = request.state.token)
        acesso_data=await api_backend.get_status(filters={},token = request.state.token)
...
#             # acesso_data = await api_backend.get_acesso(filters=request.query_params)

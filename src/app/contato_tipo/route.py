...
async def contato_tipo_list(request: Request):
    try:
        data = await api_backend.get_contato_tipo(filters={},token = request.state.token)
...
async def contato_tipo_form(request: Request):
    try:
        ...
        if(len(request.query_params) !=0 ):
            contato_tipo_data = await api_backend.get_contato_tipo(filters=request.query_params,token = request.state.token)
...
async def contato_tipo_insert(request: Request):
    try:
        ...
        contato_tipo_data = await api_backend.post_contato_tipo(data=data,token = request.state.token)
...
async def contato_tipo_update(request: Request,id:int):
    try:
        data = dict(await request.form())
        await api_backend.patch_contato_tipo(id=id,data=data,token = request.state.token)

...
async def grupo_acesso_list(request: Request):
    try:
        data = await api_backend.get_grupo_acesso(filters={},token = request.state.token)
...
async def grupo_acesso_form(request: Request):
    try:
        ...
        if(len(request.query_params) !=0 ):
            grupo_acesso_data = await api_backend.get_grupo_acesso(filters=request.query_params,token = request.state.token)         
...
async def grupo_acesso_form(request: Request):
    try:
        ...
        grupo_acesso_data = await api_backend.get_grupo_acesso(filters={"id":grupo_acesso_id},token = request.state.token)
        grupo_acesso_rota_data = await api_backend.get_grupo_acesso_rota(filters={"grupo_acesso_id":grupo_acesso_id},token = request.state.token)
...
async def grupo_acesso_insert(request: Request):
    try:
        ...
        grupo_acesso_data = await api_backend.post_grupo_acesso(data=data,token = request.state.token)
...
async def grupo_acesso_update(request: Request,id:int):
    try:
        data = dict(await request.form())
        await api_backend.patch_grupo_acesso(id=id,data=data,token = request.state.token)
